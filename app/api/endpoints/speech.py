...
async def generate_speech_internal(
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Periodic memory cleanup every 3 chunks
            if i > 0 and i % 3 == 0:
        ...
        update_tts_status(request_id, TTSStatus.ERROR, error_message=f"TTS generation failed: {e}")
        print(f"✗ TTS generation failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": {"message": f"TTS generation failed: {e}", "type": "generation_error"}}
        )
        ...
        if Config.ENABLE_MEMORY_MONITORING:
            ...
            print(f"📊 Request #{REQUEST_COUNTER} - Final memory: CPU {final_mem['cpu_memory_mb']:.1f}MB", end="")
            if torch.cuda.is_available(): print(f", GPU {final_mem['gpu_memory_allocated_mb']:.1f}MB allocated")
            else: print()
            if initial_memory is not None:
                cpu_diff = final_mem['cpu_memory_mb'] - initial_memory['cpu_memory_mb']
                print(f"📈 Memory change: CPU {cpu_diff:+.1f}MB", end="")
                if torch.cuda.is_available():
                    gpu_diff = final_mem['gpu_memory_allocated_mb'] - initial_memory['gpu_memory_allocated_mb']
